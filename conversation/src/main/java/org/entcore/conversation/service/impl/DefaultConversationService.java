...
public class DefaultConversationService implements ConversationService {
	...
	@Override
	public void forward(String forwardId, JsonObject forwardedMessage, String messageId, UserInfos user,
			Handler<Either<String, JsonObject>> result) {
		// TODO
		throw new UnsupportedOperationException("forward has not been implemented yet for neo4j");
	}
	...
	private void save(String parentMessageId, JsonObject message, UserInfos user,
		...
		if (validationError(user, m, result))
			return;
		String query;
		JsonObject params = new JsonObject().put("userId", user.getUserId()).put("folderName", "DRAFT").put("props", m)
				.put("true", true);
		if (parentMessageId != null && !parentMessageId.trim().isEmpty()) { // reply
			...
			query = "MATCH (c:Conversation)-[:HAS_CONVERSATION_FOLDER]->(f:ConversationSystemFolder) "
					...
					+ "CREATE f-[:HAS_CONVERSATION_MESSAGE]->(m:ConversationMessage {props}) " + "RETURN m.id as id";
		}
	...
	public void trash(List<String> messagesId, UserInfos user, Handler<Either<String, JsonObject>> result) {
		...
		for (String id : messagesId) {
			JsonObject params = new JsonObject().put("userId", user.getUserId()).put("messageId", id).put("true", true)
					.put("trash", "TRASH");
			b.add(query, params);
		}
	...
	public void delete(List<String> messagesId, Boolean deleteAll, UserInfos user,
		...
		String deleteAndCollectAttachments = getMessageWithAttachments + "AND NOT(m-[:HAS_CONVERSATION_MESSAGE]-()) "
				+ "DELETE aLink " + "WITH a, collect({id: a.id, size: a.size}) as attachments "
	...
	public void count(String folder, String restrain, Boolean unread, UserInfos user,
		...
		if (validationParamsError(user, result, folder))
			return;
		String condition = "";
		JsonObject params = new JsonObject().put("userId", user.getUserId()).put("folder", folder).put("true", true);
		if (unread != null) {
	...
	public void findVisibleRecipients(final String parentMessageId, final UserInfos user, final String acceptLanguage,
			String search, final Handler<Either<String, JsonObject>> result) {
		if (validationParamsError(user, result))
			return;
	...
	public void createFolder(String folderName, String parentFolderId, UserInfos user,
		...
		JsonObject newFolderProps = new JsonObject().put("id", UUID.randomUUID().toString()).put("name", folderName);
	...
	public void moveToFolder(List<String> messageIds, String folderId, UserInfos user,
		...
		String query = "MATCH (c:Conversation)-[:HAS_CONVERSATION_FOLDER]->(f:ConversationSystemFolder)-[r:HAS_CONVERSATION_MESSAGE]->(m:ConversationMessage) "
				...
				+ "OPTIONAL MATCH (m)-[i: INSIDE]->(:ConversationUserFolder)<-[:HAS_CHILD_FOLDER*0.."
				+ (maxFolderDepth - 1) + "]-(:ConversationUserFolder)<-[:HAS_CONVERSATION_FOLDER]-(c) " + "DELETE i "
				+ "SET r.insideFolder = true " + "WITH f, m "
	...
	public void backToSystemFolder(List<String> messageIds, UserInfos user,
		...
		String query = "MATCH (c:Conversation)-[:HAS_CONVERSATION_FOLDER]->(f:ConversationSystemFolder)-[r:HAS_CONVERSATION_MESSAGE]->(m:ConversationMessage) "
				+ "WHERE c.userId = {userId} AND c.active = {true} AND m.id = {messageId} " + "REMOVE r.insideFolder "
				+ "WITH m, c " + "MATCH m-[i: INSIDE]->(:ConversationUserFolder)<-[:HAS_CHILD_FOLDER*0.."
				+ (maxFolderDepth - 1) + "]-(:ConversationUserFolder)<-[:HAS_CONVERSATION_FOLDER]-(c) " + "DELETE i";
	...
	public void trashFolder(String folderId, UserInfos user, Handler<Either<String, JsonObject>> result) {
		...
		JsonObject params = new JsonObject().put("userId", user.getUserId()).put("folderId", folderId).put("true", true)
				.put("trash", "TRASH");
	...
	public void addAttachment(String messageId, UserInfos user, JsonObject uploaded,
		...
		JsonObject params = new JsonObject().put("userId", user.getUserId()).put("messageId", messageId)
				...
				.put("fileId", uploaded.getString("_id")).put("true", true).put("trash", "TRASH").put("draft", "DRAFT");
	...
	public void removeAttachment(String messageId, String attachmentId, UserInfos user,
		...
		String q3 = "MATCH (attachment:MessageAttachment)<-[attachmentLink: HAS_ATTACHMENT]-(:ConversationMessage)<-[messageLinks: HAS_CONVERSATION_MESSAGE]-(:ConversationSystemFolder) "
				...
				+ "WHERE NOT(attachment-[:HAS_ATTACHMENT]-()) " + "DELETE attachment " + "RETURN true as deletionCheck";
