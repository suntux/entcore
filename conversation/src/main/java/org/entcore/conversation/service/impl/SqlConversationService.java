...
public class SqlConversationService implements ConversationService {
	...
	private void getSenderAttachments(String senderId, String messageId, Handler<Either<String, JsonObject>> handler) {
		String query = "SELECT " + "coalesce(json_agg(distinct att.id), '[]'::json) as attachmentIds,"
				+ "coalesce(sum(att.size), 0)::integer as totalQuota " + "FROM " + attachmentTable + " att " + "JOIN "
				+ userMessageAttachmentTable + " uma " + "ON (att.id = uma.attachment_id) " + "JOIN " + userMessageTable
				+ " um " + "ON um.user_id = uma.user_id AND um.message_id = uma.message_id "
	...
	@Override
	public void send(final String parentMessageId, final String draftId, final JsonObject message, final UserInfos user,
	...
	private void sendMessage(final String parentMessageId, final String draftId, final JsonObject message,
		...
		getSenderAttachments(user.getUserId(), draftId, new Handler<Either<String, JsonObject>>() {
			public void handle(Either<String, JsonObject> event) {
				...
				String updateUnread = "UPDATE " + userMessageTable + " " + "SET unread = " + unread
	...
	@Override
	public void list(String folder, String restrain, Boolean unread, UserInfos user, int page, final String searchText,
		...
		String query = "SELECT " + messagesFields + ", um.unread as unread, "
				...
				+ " r ON um.message_id = r.parent_id AND r.from = um.user_id AND r.state= ? " + "WHERE um.user_id = ? "
				+ additionalWhere + " " + "GROUP BY m.id, unread " + "ORDER BY m.date DESC LIMIT " + LIST_LIMIT
				+ " OFFSET " + skip;

		sql.prepared(query, values,
				SqlResult.validResultHandler(results, "attachments", "to", "toName", "cc", "ccName", "displayNames"));
	}
	...
	public void listThreads(UserInfos user, int page, Handler<Either<String, JsonArray>> results) {
		...
		values.add(user.getUserId());
		String query = " WITH threads AS ( " + " SELECT * from (SELECT  DISTINCT ON (m.thread_id) thread_id AS "
				+ messagesFields + " FROM " + userMessageTable + " um " + " JOIN " + messageTable
				+ " m ON um.message_id = m.id "
				...
				+ ", unread FROM threads JOIN (SELECT m.thread_id, SUM(CASE WHEN um.unread THEN 1 ELSE 0 END) AS unread "
	...
	public void trashThread(List<String> threadIds, UserInfos user, Handler<Either<String, JsonObject>> result) {
		JsonArray values = new fr.wseduc.webutils.collections.JsonArray();
		StringBuilder query = new StringBuilder("UPDATE " + userMessageTable + " AS um  " + "SET trashed = true "
				+ "FROM conversation.messages as m " + "WHERE m.thread_id IN ");
	...
	public void delete(List<String> messagesId, Boolean deleteAll, UserInfos user,
		...
		String getTotalQuota = "SELECT coalesce(sum(um.total_quota), 0)::integer AS totalQuota FROM " + userMessageTable
				+ " um " + "WHERE um.user_id = ? AND um.trashed = true";
	...
	public void get(String messageId, UserInfos user, Handler<Either<String, JsonObject>> result) {
		...
		String selectQuery = "SELECT " + "m.*, "
				...
				+ attachmentTable + " att " + "ON att.id = uma.attachment_id " + "WHERE um.user_id = ? AND m.id = ?  "
				+ "GROUP BY m.id";
		...
		sql.transaction(builder.build(), SqlResult.validUniqueResultHandler(1, result, "attachments", "to", "toName",
				"cc", "ccName", "displayNames"));
	}
	...
	@Override
	public void findVisibleRecipients(final String parentMessageId, final UserInfos user, final String acceptLanguage,
			final String search, final Handler<Either<String, JsonObject>> result) {
		if (validationParamsError(user, result))
			return;
	...
	public void toggleUnread(List<String> messagesIds, boolean unread, UserInfos user,
		...
		JsonArray values = new fr.wseduc.webutils.collections.JsonArray();
		String query = "UPDATE " + userMessageTable + " " + "SET unread = ? " + "WHERE user_id = ? AND message_id IN "
				+ Sql.listPrepared(messagesIds.toArray());
	...
	@Override
	public void toggleUnreadThread(List<String> threadIds, boolean unread, UserInfos user,
			Handler<Either<String, JsonObject>> result) {
		JsonArray values = new fr.wseduc.webutils.collections.JsonArray();
		StringBuilder query = new StringBuilder("UPDATE " + userMessageTable + " AS um  " + "SET  unread = ? "
				+ "FROM conversation.messages as m " + "WHERE m.thread_id IN ");
		values.add(unread);
		query.append(generateInVars(threadIds, values));
	...
	public void createFolder(final String folderName, final String parentFolderId, final UserInfos user,
		...
		final JsonObject messageObj = new JsonObject().put("id", UUID.randomUUID().toString()).put("name", folderName)
				.put("user_id", user.getUserId());

		if (parentFolderId != null) {
			JsonArray values = new fr.wseduc.webutils.collections.JsonArray().add(user.getUserId()).add(parentFolderId);
	...
	public void trashFolder(String folderId, UserInfos user, Handler<Either<String, JsonObject>> result) {
		...
		JsonArray values = new fr.wseduc.webutils.collections.JsonArray().add(true).add(folderId).add(user.getUserId())
				.add(false);
	...
	public void restoreFolder(String folderId, UserInfos user, Handler<Either<String, JsonObject>> result) {
		...
		JsonArray values = new fr.wseduc.webutils.collections.JsonArray().add(false).add(folderId).add(user.getUserId())
				.add(true);
	...
	public void deleteFolder(String folderId, Boolean deleteAll, UserInfos user,
		...
		String recursiveTerm = "SELECT f.* FROM " + folderTable + " AS f JOIN " + "parents ON f.parent_id = parents.id "
				+ "WHERE f.user_id = ?";
		recursiveValues.add(user.getUserId());
		...
		String quotaRecursion = "WITH RECURSIVE parents AS ( " + nonRecursiveTerm + "UNION " + recursiveTerm + ") "
				+ "SELECT COALESCE(sum(um.total_quota), 0)::integer AS totalQuota FROM parents JOIN " + userMessageTable
				+ " um ON um.folder_id = parents.id AND um.user_id = parents.user_id ";

		builder.prepared(quotaRecursion, recursiveValues);

		/*
		 * Physically delete the folder, which will start a cascading delete process for
		 * parent folders, messages and attachments.
		 */
	...
	public void addAttachment(String messageId, UserInfos user, JsonObject uploaded,
		...
		JsonObject attParams = new JsonObject().put("id", uploaded.getString("_id"))
				...
				.put("charset", uploaded.getJsonObject("metadata").getString("charset")).put("size", attachmentSize);
	...
	public void getAttachment(String messageId, String attachmentId, UserInfos user,
		...
		String query = "SELECT att.* FROM " + attachmentTable + " att JOIN " + userMessageAttachmentTable
				+ " uma ON uma.attachment_id = att.id " + "WHERE att.id = ? AND uma.user_id = ? AND uma.message_id = ?";
	...
	@Override
	public void forward(String forwardedId, JsonObject forwardedMessage, String messageId, UserInfos user,
			Handler<Either<String, JsonObject>> result) {
		// 0 copy last message
		Future<JsonObject> copyParent = Future.future();
		// copy last message
		SqlStatementsBuilder builder = new SqlStatementsBuilder();
		String newId = UUID.randomUUID().toString();
		String insertQuery = "INSERT INTO %s(id, parent_id, subject, body, \"from\", \"fromName\", \"to\", \"toName\", cc, \"ccName\", \"displayNames\", state, date, language, text_searchable, thread_id) "
				+ "SELECT ?, NULL, subject, body, \"from\", \"fromName\", \"to\", \"toName\", cc, \"ccName\", \"displayNames\", ?, date, language, text_searchable, ? FROM %s WHERE id=?";
		insertQuery = String.format(insertQuery, messageTable, messageTable);
		JsonArray inSertValues = new fr.wseduc.webutils.collections.JsonArray();
		inSertValues.add(newId);
		inSertValues.add("FORWARD");//forward state trigger a function is postgres
		inSertValues.add(newId);
		inSertValues.add(forwardedId);
		builder.prepared(insertQuery, inSertValues);
		// copy usermessages link
		builder.insert(userMessageTable,
				new JsonObject().put("user_id", user.getUserId()).put("message_id", newId));
		//
		sql.transaction(builder.build(), SqlResult.validUniqueResultHandler(0, res -> {
			if (res.isLeft()) {
				copyParent.fail(res.left().getValue());
			} else {
				copyParent.complete(res.right().getValue());
			}
		}));
		copyParent.setHandler(resCopy -> {
			if (resCopy.failed()) {
				result.handle(new Either.Left<String, JsonObject>(resCopy.cause().getMessage()));
				return;
			}
			// 1 - forward attachments, add relationships between the message and the
			// already existing attachments
			forwardAttachments(forwardedId, messageId, user, event -> {
				if (event.isLeft()) {
					result.handle(new Either.Left<String, JsonObject>(event.left().getValue()));
					return;
				}
				// 2 update message
				String query = String.format("UPDATE %s SET thread_id=?, parent_id=? WHERE id = ? ", messageTable);
				JsonArray values = new fr.wseduc.webutils.collections.JsonArray();
				values.add(newId);
				values.add(newId);
				values.add(messageId);
				Future<JsonObject> futureSql = Future.future();
				sql.prepared(query, values, SqlResult.validUniqueResultHandler(eitherSql -> {
					if (eitherSql.isLeft()) {
						futureSql.fail(eitherSql.left().getValue());
					} else {
						futureSql.complete(eitherSql.right().getValue());
					}
				}));
				futureSql.setHandler(newMessage -> {
					if (newMessage.succeeded()) {
						result.handle(new Either.Right<String, JsonObject>(newMessage.result()));
					} else {
						result.handle(new Either.Left<String, JsonObject>(newMessage.cause().getMessage()));
					}
				});
			});
		});
