...
import org.entcore.workspace.service.impl.VideoRecorderWorker;

import static fr.wseduc.webutils.Utils.handlerToAsyncHandler;
import static fr.wseduc.webutils.Utils.isNotEmpty;
import static fr.wseduc.webutils.request.filter.UserAuthFilter.SESSION_ID;

public class VideoRecorderHandler implements Handler<ServerWebSocket> {

	private static final Logger log = LoggerFactory.getLogger(VideoRecorderHandler.class);
	private static final long TIMEOUT = 5000l;
	private final Vertx vertx;
	private final EventBus eb;

	public VideoRecorderHandler(Vertx vertx) {
		this.vertx = vertx;
	...
	public void handle(final ServerWebSocket ws) {
		...
		UserUtils.getSession(Server.getEventBus(vertx), sessionId, infos -> {
			...
			final String id = ws.path().replaceFirst("/video/", "");
			eb.send(VideoRecorderWorker.class.getSimpleName(),
					new JsonObject().put("action", "open").put("id", id), handlerToAsyncHandler(m -> {
						if ("ok".equals(m.body().getString("status"))) {
							ws.frameHandler(frame -> {
								if (frame.isBinary()) {
									log.debug("frame handler");
									eb.send(VideoRecorderWorker.class.getSimpleName() + id,
											frame.binaryData().getBytes(),
											new DeliveryOptions().setSendTimeout(TIMEOUT),
											(Handler<AsyncResult<Message<JsonObject>>>) ar -> {
												if (ar.failed() || !"ok".equals(ar.result().body().getString("status"))) {
													ws.writeTextMessage("video.chunk.error");
												}
											});
								} else {
							...
							ws.closeHandler(event -> cancel(id, null));
							ws.resume();
						} else {
	...
	private void save(String id, String name, JsonObject infos, final ServerWebSocket ws) {
		...
		eb.send(VideoRecorderWorker.class.getSimpleName(), message,
