...
public abstract class GenericShareService implements ShareService {
	...
	@Override
	public void findUserIdsForShare(JsonArray sharedArray,final String userId, Handler<AsyncResult<Set<String>>> h) {
		@SuppressWarnings("rawtypes")
		List<Future> futures = sharedArray.stream().map(o -> (JsonObject) o).map(shareRow -> {
			JsonObject json = (JsonObject) shareRow;
			if (json.getString("userId") != null) {
				Set<String> ids = new HashSet<>();
				ids.add(json.getString("userId"));
				return Future.succeededFuture(ids);
			} else {
				String groupId = json.getString("groupId");
				if (groupId != null) {
					Future<Set<String>> future = Future.succeededFuture();
					UserUtils.findUsersInProfilsGroups(groupId, eb, userId, false, ev -> {
						Set<String> ids = new HashSet<>();
						if (ev != null) {
							for (Object o : ev) {
